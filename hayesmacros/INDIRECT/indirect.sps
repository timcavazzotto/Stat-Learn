...
/* and bootstrap confidence intervals in single-step mediator models.  The syntax is */.
/* */.
...
/* list of one or more mediator variables through which the IV's effect is presumably transmitted to the */.
...
/* n is set to 1 to print normal theory standard errors for indirect effects, */.
/* t is set to 1 to do all possible pairwise contrasts between indirect effects, */.
...
/* cov defaults to 0 (meaning covlist is a null string), t defaults to 0, z defaults to 1000, ci default to 95, n defaults to 0 */.
/* Bias-corrected and accelerated confidence intervals are printed by default.  bc and bca confidence intervals */.
/*  are not printed by default.  Anything after mlist in the syntax is optional */.
/* The macro assumes missing values are represented in the data with a period or are defined by the user */.
/* Missing data will be deleted listwise prior to analysis */.
/* If dv is dichotomous then c, c', and the paths from mediators to the outcome are estimated with */.
/* logistic regression.  Otherwise, OLS is used.  The macro assumes the mediators are all quantitative, */.
/* and all paths from the iv to the mediators are estimated using OLS. Do not use this macro with categorical mediators */.
/* Normal theory tests are not available for models with covariates or models with a dichotomous dv */.
/* */.
/* Macro written by Andrew F. Hayes, The Ohio State University, afhayes@gmail.com */.
/* version 4.2, August 20, 2011 */.
...
do if (jjj > !iterate).
  ...
  do if (#d = 1).
    ...
    print/title = "http://www.afhayes.com"/space=0.
    ...
    print/title = "Also see Chapter 5 of Introduction to Mediation, Moderation, and"/space=0.
    print/title ="Conditional Process Analysis. New York: The Guilford Press"/space=0.
    print/title = "http://www.guilford.com/p/hayes3"/space=0.
    print/title = "*****************************************************************"/space=0.
   ...
   print pf/title = "Logistic Regression Summary for DV Model"/clabels = "-2LL" "Model LL" "McFadden" "CoxSnell" "Nagelkrk" "n"/format F10.4.
    ...
    do if (!normal <> 0 and !c = 0 and ovals <> 2).
...
do if ((nv-nc-2) > 1 and (!contrast = 1) and ((!normal = 1 and !c = 0) OR btn > 999))).
print/title = "*****************************************************************".
...
Print/title = "Bootstrap confidence intervals are preferred to normal theory tests for"/space=0.
